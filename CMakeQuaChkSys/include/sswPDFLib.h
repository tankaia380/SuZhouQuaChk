...
#pragma message("Automatically Linking With MPDFLib.dll")
#endif 
...
public:
	...
	bool AddImage(const char* strImageFilePath, float fScale = 1.0, SSW_PDF_ContentStruct eContentStruct = SSW_CENTER, int nPosX = 0, int nPosY = 0, int nWidth = 0, int nHeight = 0, bool bAddLabel = false, char**pLabelStr = NULL, int nLabelCount = 0);
...
// 		float *pg = NULL, float *pb = NULL, SSW_PDF_ContentStruct eContent = SSW_CENTER);
	bool AddList(int nCols, int nRows,vector<CString> pListInfo);
